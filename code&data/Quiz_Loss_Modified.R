# Read Fannie acquisition and performance data, write into rda files
# This is based on Loss_101.R from Fannie's web site.

library(data.table)
library(zoo)

# Create function to handle missing Current UPBs '
#in the last record by setting them to the record prior
na.lomf <- function(x) {
...
setwd("D:/MQM530/Assignment/")
  
aqtr   <- "2000Q1"
fadata <- paste0("adata_",aqtr,".rda")
fcdata <- paste0("cdata_",aqtr,".rda")
fpdata <- paste0("pdata_",aqtr,".rda")
acqfile<- paste0("Acquisition_",aqtr,".txt")
pfmfile<- paste0("Performance_",aqtr,".txt")

# Read Fannie quarterly acquisition data
...
Data_A<- fread(acqfile, sep = "|", colClasses = Acquisition_ColClasses, showProgress=FALSE)
setnames(Data_A, Acquisitions_Variables)
setkey(Data_A, "LOAN_ID")

# remove Product Type
Data_A$Product.Type <- NULL
Data_A$ORIG_DTE<-as.Date(paste0("01/",Data_A$ORIG_DTE), "%d/%m/%Y")
Data_A$FRST_DTE<-as.Date(paste0("01/",Data_A$FRST_DTE), "%d/%m/%Y")
...
save(Data_A,file=fadata)

# Read Fannie quarterly performance data
...
Data_P = fread(pfmfile, sep = "|", colClasses = Performance_ColClasses, showProgress=FALSE)
...
# Standardize Delinquency Status Codes -- convert to numeric and set "X" to NA
Data_P$Delq.Status<-as.numeric(ifelse(Data_P$Delq.Status=="X", "999", Data_P$Delq.Status))
Data_P$Delq.Status<-ifelse(Data_P$Delq.Status==999,NA,Data_P$Delq.Status)
...
# Fill in missing UPB in first few months in Data_P with the origination UPB in Data_A
# Add Original Rate from the Acquisitions Files
Data_P[Data_A, ORIG_AMT:=i.ORIG_AMT, allow.cartesian=TRUE]
Data_P$LAST_UPB <- ifelse(is.na(Data_P$LAST_UPB),Data_P$ORIG_AMT,Data_P$LAST_UPB)

save(Data_P,file=fpdata)

# create combined data file
Data_P$ORIG_AMT<- NULL
...
Data_C[,c("VinYr", "ActYr", "DispYr", "F180_UPB", "FCE_UPB") :=list(format(as.yearmon(ORIG_DTE, format="%m/%Y"), "%Y"),
...
Data_C[,c("MODIR_COST","MODFB_COST"):=
...
Data_C[, MODTOT_COST :=rowSums(.SD, na.rm = TRUE), .SDcols = c("modfg_cost", "MODIR_COST","MODFB_COST")]
Data_C[,c("SPDelq1","SPDelq2", "CountF1", "CountFC", "modir_cost", "modfb_cost"):=NULL]

save(Data_C,file=fcdata)



##### ia_6
## q1
loans <- Data_C[ORIG_TRM==360,]
dateloans <- loans$ORIG_DTE
a1 = min(dateloans) 

##q2
a <- c("02", "03", "09","15" )
defaultedstatus <- loans[which(loans$Zero.Bal.Code %in% a)]
defaultedstatus<- defaultedstatus[which (!is.na(defaultedstatus$DISP_DT))]
nrow(defaultedstatus)

##q3
head(defaultedstatus$LAST_UPB)
sum(defaultedstatus$LAST_UPB)

##q4
head(defaultedstatus$NET_LOSS)
sum(defaultedstatus$NET_LOSS)

##q5
recrt <- 1-(defaultedstatus$NET_LOSS/defaultedstatus$LAST_UPB)
head(recrt)
m <-median(recrt)
round(m,4)
